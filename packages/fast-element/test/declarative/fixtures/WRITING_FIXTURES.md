...
declarative docs](../../../docs/declarative/syntax.md#syntax).
...
> **Tip:** If you're iterating on a fixture, use `npm run test-server:declarative -w @microsoft/fast-element` to start the source-based Vite server with file watching. But remember to run `build:fixtures` before committing — the Vite dev server uses the generated `index.html`.
